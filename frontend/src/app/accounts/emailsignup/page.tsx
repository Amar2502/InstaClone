...
export default function EmailSignup() {
  ...
  const validatePassword = (password: string) => /^(?=.*[A-Za-z])(?=.*\d).+$/.test(password);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    const emailValid = validateEmail(register.email_num);
    const passwordValid = validatePassword(register.password);

    setError({
      email_num: emailValid ? "" : "Enter a valid email address",
      ...
      <div className="w-full max-w-sm">
        <div className=" border border-zinc-800 rounded-sm p-8 flex flex-col items-center gap-4 mt-5">
          ...
          <form onSubmit={handleSubmit} className="w-full flex flex-col gap-2">
            ...
            />
            {error.email_num && (
              <p className="text-xs text-red-500">{error.email_num}</p>
            )}
